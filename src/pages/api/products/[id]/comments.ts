  ...
  if (req.method === "GET") {
    try {
      ...
      if (error instanceof AxiosError) {
        console.error("상품 댓글 목록 조회 실패 상세:");
      ...
      console.error("상품 댓글 목록 조회 실패:", error);
      return res.status(500).json({ message: "상품 댓글 목록 조회 실패" });
    }
    ...
    try {
      ...
      if (error instanceof AxiosError) {
        console.error("상품 댓글 등록 실패:", error.response?.data);
        return res.status(error.response?.status || 500).json({
          message: "상품 댓글 등록 실패",
          error: error.response?.data || error.message,
        });

...
const Signup: React.FC = () => {
  ...
  useEffect(() => {
    validateForm();
  }, [email, nickname, password, passwordRepeat]);
  ...
  const validateForm = () => {
    ...
    if (!passwordRepeat) {
      newErrors.passwordRepeat = '비밀번호 확인을 입력해 주세요';
    } else if (password !== passwordRepeat) {
      newErrors.passwordRepeat = '비밀번호가 일치하지 않습니다';
    }
        ...
        <div>
          ...
          <div className='mt-1 relative rounded-md shadow-sm'>
            ...
            <button
              ...
              className='absolute inset-y-0 right-0 pr-3 flex items-center'
            >
              {showPasswordRepeat ? <Invisible /> : <Visible />}
            </button>
          </div>
          {touched.passwordRepeat && errors.passwordRepeat && (
            <p className='mt-1 text-sm text-red-600'>{errors.passwordRepeat}</p>
          )}
        </div>
        <button
          type='submit'
